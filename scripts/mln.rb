...
module MLN
   ...
   #    {:name => '', :predicate => '', :hasTruth => true/false, :defaultTruth => value},
   # ]
   ...
   def MLN.generateDataFile(dataDir, outPath, fileInfo)
      ...
      File.open(outPath, 'w'){|outFile|
         fileInfo.each{|fileInfo|
            translateDataFiles(File.join(dataDir, fileInfo[:name]), fileInfo[:predicate], fileInfo[:hasTruth], fileInfo[:defaultTruth], outFile)
         }
      }
   end

   def MLN.translateDataFiles(path, predicate, hasTruth, defaultTruth, outFile)
