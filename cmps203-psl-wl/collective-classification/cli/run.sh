...
readonly BASE_NAME='collective-classification'

readonly ADDITIONAL_PSL_OPTIONS='-D categoricalevaluator.categoryindexes=1 -D weightlearning.evaluator=CategoricalEvaluator -D categoricalevaluator.defaultpredicate=HasCat'
readonly ADDITIONAL_LEARN_OPTIONS='--learn org.linqs.psl.application.learning.weight.search.InitialWeightHyperband'
...
function runWeightLearning() {
   ...
   java -Xmx25G -Xms25G -jar "${JAR_PATH}" --model "${BASE_NAME}.psl" --data "${BASE_NAME}-learn.data" ${ADDITIONAL_LEARN_OPTIONS} ${ADDITIONAL_PSL_OPTIONS} "$@"
   if [[ "$?" -ne 0 ]]; then
...
function runEvaluation() {
   ...
   java -Xmx25G -Xms25G -jar "${JAR_PATH}" --model "${BASE_NAME}.psl" --data "${BASE_NAME}.data" --output inferred-predicates ${ADDITIONAL_EVAL_OPTIONS} ${ADDITIONAL_PSL_OPTIONS} "$@"
   #java -Xmx25G -Xms25G -jar "${JAR_PATH}" --model "${BASE_NAME}-learned.psl" --data "${BASE_NAME}-eval.data" --output inferred-predicates ${ADDITIONAL_EVAL_OPTIONS} ${ADDITIONAL_PSL_OPTIONS} "$@"
   if [[ "$?" -ne 0 ]]; then
